extends Node2D
## Dark forest undergrowth: moss, thorns, blood-roses, toadstools — not pastel fairy land.


const LEGEND_GREEN := Color(0.28, 0.42, 0.28)
const LEGEND_MOSS := Color(0.22, 0.36, 0.24)
const BLOOM_BLOOD := Color(0.72, 0.28, 0.22)
const BLOOM_RUST := Color(0.65, 0.38, 0.2)
const BLOOM_AMBER := Color(0.78, 0.62, 0.28)
const BLOOM_PALE := Color(0.82, 0.8, 0.72)
const LEGEND_FERN := Color(0.2, 0.38, 0.26)
...
func paint(parent: Node2D) -> void:
	...
	# Sparse meadow ring around the Lightwell — moss and blood-roses, not confetti
	_meadow_ring(parent, Vector2(0, 40), 120.0, 280.0, 48, rng)
	if PathNetwork:
		for lane in PathNetwork.lanes:
			var pts: PackedVector2Array = lane
			for i in range(pts.size() - 1):
				_path_edge_flowers(parent, pts[i], pts[i + 1], rng)
	# Toadstool rings (fewer, darker)
...
func _meadow_ring(parent: Node2D, center: Vector2, r0: float, r1: float, count: int, rng: RandomNumberGenerator) -> void:
	for i in count:
		...
		match rng.randi() % 5:
			0, 1:
				_flower(parent, pos, BLOOM_BLOOD if rng.randf() < 0.55 else BLOOM_RUST, rng.randf_range(0.7, 1.3))
			2:
				_flower(parent, pos, BLOOM_AMBER, rng.randf_range(0.6, 1.1))
			3:
				_flower(parent, pos, BLOOM_PALE, rng.randf_range(0.8, 1.2))
			_:
...
func _fairy_ring(parent: Node2D, center: Vector2, radius: float, rng: RandomNumberGenerator) -> void:
	...
	for i in n:
		...
		if rng.randf() < 0.7:
			...
			_flower(parent, pos, BLOOM_PALE if rng.randf() < 0.5 else BLOOM_BLOOD, rng.randf_range(0.7, 1.1))
	var glow := FX.make_ellipse_poly(radius * 0.7, radius * 0.45, 28, Color(0.35, 0.28, 0.18, 0.1))
...
func _scatter_glow_blossoms(parent: Node2D, rng: RandomNumberGenerator) -> void:
	# Sparse night blooms — amber / blood, not lilac glitter
	for i in 22:
		var ang := rng.randf() * TAU
...
func _pollen_clouds(parent: Node2D) -> void:
	# Dust and spores — muted amber/green only
	var colors := [
		Color(0.75, 0.65, 0.35, 0.4),
		Color(0.45, 0.55, 0.35, 0.35),
	]
	for i in 2:
		var p := FX.spark_particles(parent, colors[i], 16, "star")
...
func _flower_into(parent: Node2D, local_pos: Vector2, color: Color, sc: float) -> void:
	...
	# Center
	var core := FX.make_ellipse_poly(2.5, 2.2, 10, BLOOM_AMBER)
